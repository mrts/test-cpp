...
#include <testcpp/detail/general/release_assert.h>
...
public:
    ...
    void reset(T* p = 0)
    {
        TESTCPP_RELEASE_ASSERT(p == 0 || p != px,
                "Self-reset in scoped_pointer"); // catch self-reset errors
        this_type(p).swap(*this);
    }

    T& operator*() const
    {
        TESTCPP_RELEASE_ASSERT(px != 0,
                "Dereferencing null pointer in scoped_pointer");
        return *px;
    }
